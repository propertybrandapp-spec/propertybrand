...
export async function fetchSavedPropertyIds(clientId) {
  const { data, error } = await safeQuery(
    supabase.from("saved_properties").select("listing_id").eq("client_id", clientId)
  );
...
export async function saveProperty(clientId, listingId) {
  const { error } = await safeQuery(
    supabase.from("saved_properties").insert({ client_id: clientId, listing_id: listingId })
  );
  return { error };
}
...
export async function unsaveProperty(clientId, listingId) {
  const { error } = await safeQuery(
    supabase.from("saved_properties").delete().eq("client_id", clientId).eq("listing_id", listingId)
  );
  return { error };
}
...
export async function fetchSavedPostIds(clientId) {
  const { data, error } = await safeQuery(
    supabase.from("saved_posts").select("post_id").eq("client_id", clientId)
  );
  if (error) return { data: [], error };
...
export async function savePost(clientId, postId) {
  const { error } = await safeQuery(
    supabase.from("saved_posts").insert({ client_id: clientId, post_id: postId })
  );
  return { error };
}
...
export async function unsavePost(clientId, postId) {
  const { error } = await safeQuery(
    supabase.from("saved_posts").delete().eq("client_id", clientId).eq("post_id", postId)
  );
  return { error };
}
