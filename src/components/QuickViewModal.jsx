    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: "rgba(15,23,42,0.6)" }} onClick={onClose}>
      ...
      >
        <div className="relative">
          ...
          {property.badge && (
            <span className="absolute top-3 left-3 text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background: property.badgeColor || "#1565C0", color: "#FFFFFF" }}>
              {property.badge}
            </span>
          )}
          {images.length > 1 && (
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3 mb-1">
            ...
            {property.verified && (
              <span className="flex items-center gap-1 text-[10px] font-bold shrink-0 mt-1" style={{ color: "#1565C0" }}>
          ...
          <p className="text-2xl font-extrabold mb-4" style={{ color: "#1565C0" }}>{property.price}</p>
          ...
          {property.amenities?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-5">
              {property.amenities.slice(0, 6).map((a) => (
                <span key={a} className="text-[10px] font-semibold px-2.5 py-1 rounded-full" style={{ background: "#EFF6FF", color: "#1565C0" }}>{a}</span>
              ))}
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            ...
            <button
              ...
              style={{ background: "#FFFFFF", color: isPropertySaved(id) ? "#1565C0" : "#1F2937", border: "1px solid #E2E8F0" }}
            >
