...
function HeartBtn({ id }) {
  ...
  return (
    <button
      ...
      className="absolute top-2.5 right-2.5 w-7 h-7 rounded-full bg-white/90 backdrop-blur flex items-center justify-center shadow hover:scale-110 transition z-10"
    >
      <svg
        className={`w-3.5 h-3.5 transition-colors ${liked ? "fill-red-500 text-red-500" : "fill-none text-gray-500"}`}
        stroke="currentColor"
        strokeWidth={2}
        viewBox="0 0 24 24"
      >
...
function ImageCarousel({ images, imgCount, badge, badgeColor }) {
  ...
  return (
    <div className="relative w-full h-44 overflow-hidden rounded-t-xl group/img bg-gray-100">
      <img
        src={images[current]}
        alt="property"
      ...
      {images.length > 1 && (
        <>
          <button
            onClick={prev}
            className="absolute left-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/50 text-white text-xs flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition"
          >
            ‹
          </button>
          <button
            onClick={next}
            className="absolute right-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-black/50 text-white text-xs flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition"
          >
            ›
          </button>
        </>
      ...
      {/* Image count pill */}
      <div className="absolute bottom-2 left-2.5 flex items-center gap-1 bg-black/60 text-white text-[11px] font-semibold px-2 py-0.5 rounded-full">
...
function PropertyCard({ property }) {
  ...
  return (
    <div className="relative bg-white rounded-xl border border-gray-200 hover:shadow-xl transition-shadow duration-200 cursor-pointer group flex-shrink-0 w-64">
      {featured && (
        <div className="absolute -top-px left-0 right-0 h-0.5 bg-gradient-to-r from-yellow-400 via-[#E03A3C] to-yellow-400 rounded-t-xl z-10" />
      )}
      ...
      <div className="p-3.5">
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          {verified && (
            <span className="shrink-0 flex items-center gap-0.5 text-[10px] text-green-600 font-bold">
              <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex items-start gap-1 mb-2.5">
          <svg className="w-3 h-3 text-[#E03A3C] mt-0.5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex flex-wrap gap-1 mb-3">
          {amenities.slice(0, 3).map((a) => (
            <span key={a} className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded font-medium">
              {a}
            </span>
          ))}
          {amenities.length > 3 && (
            <span className="text-[10px] text-[#E03A3C] font-semibold px-1">
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-gray-100">
          <div className="text-[11px] text-gray-400">
            By <span className="font-semibold text-gray-600">{postedBy}</span>
            {" · "}
            {postedDays === 0 ? "Today" : `${postedDays}d ago`}
          </div>
          <button className="flex items-center gap-1 text-[11px] font-bold text-[#E03A3C] hover:underline">
...
function ScrollRow({ properties }) {
  ...
  return (
    <div className="relative group/row">
      ...
      <button
        onClick={() => scroll(-1)}
        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 w-9 h-9 rounded-full bg-white border border-gray-200 shadow-lg flex items-center justify-center text-gray-600 hover:text-[#E03A3C] hover:border-[#E03A3C] opacity-0 group-hover/row:opacity-100 transition"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
      ...
      <button
        onClick={() => scroll(1)}
        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 w-9 h-9 rounded-full bg-white border border-gray-200 shadow-lg flex items-center justify-center text-gray-600 hover:text-[#E03A3C] hover:border-[#E03A3C] opacity-0 group-hover/row:opacity-100 transition"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
        </svg>
  ...
  return (
    <section className="bg-white py-10 px-4">
      <div className="max-w-7xl mx-auto">
          ...
          {FILTER_TABS.map((tab) => (
            <button
              ...
              className={`shrink-0 px-4 py-1.5 rounded-full text-sm font-semibold border transition-all ${
                activeFilter === tab
                  ? "bg-[#E03A3C] text-white border-[#E03A3C] shadow"
                  : "bg-white text-gray-600 border-gray-300 hover:border-[#E03A3C] hover:text-[#E03A3C]"
              }`}
            >
              {tab}
            </button>
        ...
        {/* ── Bottom CTA Banner ── */}
        <div className="rounded-2xl bg-gradient-to-r from-[#E03A3C] to-rose-700 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
          ...
          <div className="flex gap-3 shrink-0">
            <button className="bg-white text-[#E03A3C] text-sm font-bold px-5 py-2.5 rounded-lg hover:bg-yellow-50 transition shadow">
              Schedule Site Visit
            </button>
            <button className="bg-white/20 text-white text-sm font-bold px-5 py-2.5 rounded-lg hover:bg-white/30 transition border border-white/40">
              Talk to Expert
            </button>
          </div>
        </div>
