...
function BlogCard({ post }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow duration-200 cursor-pointer group flex flex-col">
      {/* Image */}
      <div className="relative h-44 overflow-hidden">
        ...
        <button
          ...
          className="absolute top-3 right-3 w-7 h-7 rounded-full bg-white/90 flex items-center justify-center hover:scale-110 transition"
        >
          <svg
            className={`w-3.5 h-3.5 ${bookmarked ? "fill-[#E03A3C] text-[#E03A3C]" : "fill-none text-gray-500"}`}
      ...
      <div className="p-4 flex flex-col flex-1">
        <h3 className="text-sm font-bold text-gray-900 leading-snug mb-2 group-hover:text-[#E03A3C] transition-colors line-clamp-2">
          {post.title}
        </h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          <div className="flex items-center gap-1.5">
            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-[#E03A3C] to-rose-400 flex items-center justify-center text-white text-[8px] font-bold">
              {post.author.charAt(0)}
            </div>
            <span className="text-[10px] text-gray-500 font-medium">{post.author}</span>
          </div>
    ...
    <div className="relative rounded-2xl overflow-hidden cursor-pointer group h-72">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-gray-300 text-[10px]">{article.date}</span>
          <span className="text-gray-400 text-[10px]">·</span>
          <span className="text-gray-300 text-[10px]">{article.readTime}</span>
        </div>
        <h2 className="text-white text-lg font-extrabold leading-snug mb-2 line-clamp-2 group-hover:text-yellow-200 transition-colors">
          {article.title}
        </h2>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#E03A3C] to-rose-400 flex items-center justify-center text-white text-[9px] font-bold">
              {article.author.charAt(0)}
            </div>
            <div>
              <p className="text-white text-[11px] font-semibold">{article.author}</p>
              <p className="text-gray-400 text-[10px]">{article.authorRole}</p>
            </div>
          </div>
          <button className="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white text-xs font-semibold px-3 py-1.5 rounded-full border border-white/30 transition">
            Read Full Article
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden h-full">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 bg-[#E03A3C] rounded-full animate-pulse" />
          <h3 className="text-sm font-bold text-gray-900">Live Market News</h3>
        </div>
        <a href="#" className="text-xs font-semibold text-[#E03A3C] hover:underline">View all</a>
      </div>

      {/* News items */}
      <div className="divide-y divide-gray-50">
        {NEWS_ITEMS.map((item) => (
          <div key={item.id} className="px-5 py-3.5 hover:bg-gray-50 transition cursor-pointer group">
            <div className="flex items-start gap-3">
              <span className={`shrink-0 text-[9px] font-bold px-2 py-0.5 rounded-full mt-0.5 ${item.tagColor}`}>
                {item.tag}
              </span>
              <div className="min-w-0">
                <p className="text-xs font-semibold text-gray-800 leading-snug group-hover:text-[#E03A3C] transition line-clamp-2">
                  {item.headline}
                </p>
      ...
      <div className="px-5 py-4 bg-gray-50 border-t border-gray-100">
        <p className="text-xs font-semibold text-gray-700 mb-2">Get daily market updates</p>
        <div className="flex gap-2">
          <input
            type="email"
            placeholder="your@email.com"
            className="flex-1 text-xs border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-[#E03A3C] bg-white"
          />
          <button className="bg-[#E03A3C] text-white text-xs font-bold px-3 py-2 rounded-lg hover:bg-red-700 transition shrink-0">
            Subscribe
          </button>
        </div>
      </div>
  ...
  return (
    <section className="bg-gray-50 py-10 px-4">
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {QUICK_GUIDES.map((g) => (
              <a
                key={g.label}
                href={g.link}
                className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-[#E03A3C] hover:bg-red-50 transition group"
              >
                <span className="text-xl shrink-0">{g.icon}</span>
                <span className="text-sm font-semibold text-gray-700 group-hover:text-[#E03A3C] transition leading-snug">
                  {g.label}
                </span>
                <svg className="w-4 h-4 text-gray-300 group-hover:text-[#E03A3C] ml-auto shrink-0 transition" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                </svg>
              </a>
            ))}
        ...
        {/* ── Newsletter CTA ── */}
        <div className="bg-gradient-to-r from-[#E03A3C] to-rose-700 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
          ...
          <div className="flex gap-2 w-full md:w-auto shrink-0">
            <input
              ...
              className="flex-1 md:w-64 text-sm border-0 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-white/50 bg-white/20 text-white placeholder-red-200"
            />
            <button className="bg-white text-[#E03A3C] text-sm font-bold px-5 py-3 rounded-lg hover:bg-yellow-50 transition shrink-0 shadow">
              Subscribe
            </button>
          </div>
        </div>
