...
const AVATAR_COLORS = ["bg-blue-600", "bg-rose-600", "bg-emerald-600", "bg-violet-600", "bg-[#F59E0B]", "bg-pink-600"];
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg
          key={s}
          className={`w-3 h-3 ${s <= Math.round(rating) ? "text-[#D4AF37] fill-[#D4AF37]" : "text-gray-200 fill-gray-200"}`}
          viewBox="0 0 20 20"
        >
    ...
    <div className="flex-shrink-0 w-64 bg-[#FFFFFF] border border-[#E2E8F0] rounded-xl p-4 hover:shadow-xl transition-shadow duration-200 cursor-pointer group">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="min-w-0">
          <p className="text-sm font-bold text-[#1F2937] truncate">{agent.name}</p>
          <p className="text-[11px] text-[#6B7280] truncate">{agent.agency || agent.city || "Independent Agent"}</p>
          {agent.rating > 0 && (
      ...
      {/* Meta row */}
      <div className="text-[11px] text-[#6B7280] mb-3 flex items-center gap-1 flex-wrap">
        {agent.since && agent.since !== "—" && (
          <>
            <svg className="w-3 h-3 text-[#6B7280] shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Since <span className="font-semibold text-[#1F2937] ml-0.5">{agent.since}</span>
          </>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-2 mb-3">
        <div className="bg-[#F8FAFC] rounded-lg p-2 text-center">
          <p className="text-base font-extrabold text-[#1F2937]">{agent.deals}</p>
          <p className="text-[10px] text-[#6B7280] font-medium leading-tight">Deals Closed</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-[#E2E8F0]">
        {NEW_PROJECT_FEATURES.map((feat) => (
          ...
          >
            ...
            <span className="text-sm font-semibold text-[#1F2937] text-center leading-tight">
              {feat.label}
            </span>
          </button>
        ))}
    ...
    <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-[radial-gradient(circle_at_top_right,_rgba(30,136,229,0.08),_transparent_35%),linear-gradient(to_bottom_right,#ffffff,#f8fafc)] shadow-lg p-6 md:p-8">
      ...
      <div className="relative flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
        ...
        <div className="flex-1 max-w-md">
          ...
          <h3 className="text-[#1F2937] text-2xl font-extrabold leading-tight mb-2">
        ...
        <div className="flex flex-col sm:flex-row gap-6 flex-1">
          {SITE_VISIT_STEPS.map((step, i) => (
            ...
            >
              ...
              <div>
                <p className="text-[#1F2937] text-sm font-bold leading-tight">
                  {step.label}
                </p>
        ...
        <div>
          ...
          {loading ? (
            <div className="flex gap-4 overflow-hidden">
              {[1, 2, 3].map((i) => <div key={i} className="w-64 h-64 rounded-xl shrink-0 animate-pulse" style={{ background: "#F1F5F9" }} />)}
            </div>
          ) : agents.length === 0 ? (
